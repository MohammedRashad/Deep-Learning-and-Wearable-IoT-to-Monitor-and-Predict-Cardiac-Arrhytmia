...
public class DoUtils {
    private static final String[] svgNames = {
    ...
    private static final HashMap<String,Integer> svgColors = new HashMap<String,Integer>();
    // parse SVG color name to RGB int
    public static int parseColor(String c){
        if(svgColors.isEmpty())
            for(int i=0; i<svgNames.length; i+=2)
                svgColors.put(svgNames[i], android.graphics.Color.parseColor(svgNames[i+1]));
        if(svgColors.containsKey(c)) return svgColors.get(c);
        else { try { return android.graphics.Color.parseColor(c);
        } catch(Exception e) { return 0; }}
    }
    // convert to HEX string
    public static String hexColor(int c){
